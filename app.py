...
st.set_page_config(
    page_title="QuLab: Portfolio AI Performance & Benchmarking Dashboard", layout="wide")
...
st.sidebar.divider()
st.markdown("# QuLab: Portfolio AI Performance & Benchmarking Dashboard")
st.divider()
...
num_companies = st.sidebar.number_input(
    "Number of Portfolio Companies", min_value=5, max_value=20, value=10, key="num_companies_input")
num_quarters = st.sidebar.number_input(
    "Number of Quarters (History)", min_value=2, max_value=10, value=5, key="num_quarters_input")
...
def load_portfolio_data(num_companies_val, num_quarters_val):
    ...
    for q_idx in range(num_quarters_val):
        ...
        for i, company_name in enumerate(companies):
            ...
            ai_investment = np.random.uniform(
                0.5, 10.0) * 1_000_000  # in millions
            ebitda_impact = np.random.uniform(0.5, 8.0)  # Percentage impact
            baseline_ebitda = np.random.uniform(
                50, 500) * 1_000_000  # in millions
            ...
            gamma_coefficient = np.random.uniform(
                0.05, 0.15)  # For EBITDA attribution
            ai_premium_coefficient = np.random.uniform(
                0.01, 0.05)  # For exit multiple premium
            baseline_multiple = np.random.uniform(
                8.0, 15.0)  # Industry baseline multiple
    ...
    df['IndustryMeanOrgAIR'] = df.groupby(['Industry', 'Quarter'], observed=False)[
        'SystematicOpportunity'].transform('mean')
    df['IndustryStdDevOrgAIR'] = df.groupby(['Industry', 'Quarter'], observed=False)[
        'SystematicOpportunity'].transform('std').fillna(5)
    ...
    df['Quarter'] = pd.Categorical(
        df['Quarter'], categories=quarter_categories, ordered=True)
...
def calculate_org_ai_r(df, alpha=0.6, beta=0.15):
    ...
    df_copy['Org_AI_R_Score'] = np.clip(
        df_copy['Org_AI_R_Score'], 0, 100)  # Clip to a 0-100 range
    return df_copy
...
def calculate_benchmarks(df):
    ...
    df_copy['Org_AI_R_Percentile'] = df_copy.groupby('Quarter', observed=False)[
        'Org_AI_R_Score'].rank(pct=True) * 100

    # Update IndustryMeanOrgAIR based on the newly calculated Org_AI_R_Score
    df_copy['IndustryMeanOrgAIR'] = df_copy.groupby(
        ['Industry', 'Quarter'], observed=False)['Org_AI_R_Score'].transform('mean')
    ...
    def safe_zscore_transform(series):
        ...
        # Return Series of 0.0 with original index
        return pd.Series(0.0, index=series.index)

    df_copy['Org_AI_R_Z_Score'] = df_copy.groupby(['Industry', 'Quarter'], observed=False)[
        'Org_AI_R_Score'].transform(safe_zscore_transform)
...
def calculate_aie_ebitda(df):
    ...
    df_sorted['Delta_Org_AI_R'] = df_sorted.groupby('CompanyID', observed=False)[
        'Org_AI_R_Score'].diff().fillna(0)

    # AI Investment Efficiency (AIE)
    df_sorted['AI_Investment_Efficiency'] = df_sorted.apply(
        # Scale for readability (e.g., impact points per million invested)
        lambda row: (row['Delta_Org_AI_R'] / row['AI_Investment']
                     ) * row['EBITDA_Impact'] * 1_000_000
    ...
    df_sorted['Attributed_EBITDA_Impact_Pct'] = df_sorted.apply(
        lambda row: row['GammaCoefficient'] * row['Delta_Org_AI_R'] *
        (row['IndustryMeanOrgAIR'] / 100)  # H^R_org,k as a % from 100
        if row['Delta_Org_AI_R'] > 0 else 0, axis=1
    )
    df_sorted['Attributed_EBITDA_Impact_Absolute'] = (
        df_sorted['Attributed_EBITDA_Impact_Pct'] / 100) * df_sorted['BaselineEBITDA']
...
def identify_actionable_insights(df, org_ai_r_threshold_coe, ebitda_impact_threshold_coe,
    ...
    latest_data = df.loc[df.groupby('CompanyID', observed=False)[
        'Quarter'].idxmax()]
...
def calculate_exit_readiness_and_valuation(df, w1=0.35, w2=0.40, w3=0.25):
    ...
    df_copy['Exit_AI_R_Score'] = (
        w1 * df_copy['Visible'] + w2 *
        df_copy['Documented'] + w3 * df_copy['Sustainable']
    )
    df_copy['Exit_AI_R_Score'] = np.clip(df_copy['Exit_AI_R_Score'], 0, 100)

    # Calculate AI Premium and Projected Exit Multiple
    df_copy['AI_Premium_Multiple_Additive'] = df_copy['AI_PremiumCoefficient'] * \
        df_copy['Exit_AI_R_Score'] / 100
    df_copy['Projected_Exit_Multiple'] = df_copy['BaselineMultiple'] + \
        df_copy['AI_Premium_Multiple_Additive']
...
if "portfolio_df" not in st.session_state:
    ...
    st.session_state.portfolio_df = load_portfolio_data(
        default_num_companies, default_num_quarters)
    # Perform initial calculations immediately after data load
    st.session_state.portfolio_df = calculate_org_ai_r(
        st.session_state.portfolio_df)
    st.session_state.portfolio_df = calculate_benchmarks(
        st.session_state.portfolio_df)
    st.session_state.portfolio_df = calculate_aie_ebitda(
        st.session_state.portfolio_df)
    st.session_state.portfolio_df = calculate_exit_readiness_and_valuation(
        st.session_state.portfolio_df)
...
if st.sidebar.button("Generate New Portfolio Data", key="generate_data_button", help="Click to create a new synthetic dataset based on the parameters above. All subsequent calculations will use this data."):
    ...
    st.session_state.portfolio_df = calculate_exit_readiness_and_valuation(
        st.session_state.portfolio_df)
    st.success(
        "New synthetic portfolio data generated successfully! All calculations have been re-run.")
...
# Page 2: Calculating Org-AI-R Scores
    ...
    *   $\text{Synergy}(V^R_{org,j}, H^R_{org,k})$: A term representing the alignment and integration between a company's idiosyncratic readiness and the systematic opportunity in its industry.
    ...
    alpha = st.slider("Weight for Idiosyncratic Readiness ($\\alpha$)", min_value=0.55, max_value=0.70, value=0.60, step=0.01, key="alpha_slider",
    ...
    beta = st.slider("Synergy Coefficient ($\\beta$)", min_value=0.08, max_value=0.25, value=0.15, step=0.01, key="beta_slider",
    ...
    if st.button("Recalculate Org-AI-R Scores", key="recalculate_org_ai_r_button", help="Click to re-compute Org-AI-R scores with the selected weights."):
        st.session_state.portfolio_df = calculate_org_ai_r(
            st.session_state.portfolio_df, alpha, beta)
        st.session_state.portfolio_df = calculate_benchmarks(
            st.session_state.portfolio_df)  # Recalculate benchmarks after Org-AI-R changes
        st.session_state.portfolio_df = calculate_aie_ebitda(
            st.session_state.portfolio_df)  # Recalculate AIE/EBITDA impact
        st.session_state.portfolio_df = calculate_exit_readiness_and_valuation(
            st.session_state.portfolio_df)  # Re-calculate exit values to ensure consistency
        st.success("Org-AI-R scores and related metrics updated successfully!")

    st.subheader("Latest Quarter's PE Org-AI-R Scores:")
    if not st.session_state.portfolio_df.empty:
        latest_quarter_df = st.session_state.portfolio_df[st.session_state.portfolio_df['Quarter']
                                                          == st.session_state.portfolio_df['Quarter'].max()]
        st.dataframe(latest_quarter_df[['CompanyName', 'Industry', 'Org_AI_R_Score']].sort_values(
            by='Org_AI_R_Score', ascending=False).set_index('CompanyName'))
    else:
        # Render an empty dataframe with expected columns for AppTest compatibility
        empty_df = pd.DataFrame(
            columns=['CompanyName', 'Industry', 'Org_AI_R_Score']).set_index('CompanyName')
...
# Page 3: Benchmarking AI Performance
    ...
    **Org-AI-R Z-Score:**
    ...
    if not st.session_state.portfolio_df.empty:
        quarter_options = st.session_state.portfolio_df['Quarter'].unique(
        ).tolist()

        # Determine the index of the latest quarter for the selectbox default
        selected_quarter_index = len(
            quarter_options) - 1 if quarter_options else 0
        ...
        sns.barplot(x='CompanyName', y='Org_AI_R_Score', hue='Industry', data=filtered_df.sort_values(
            by='Org_AI_R_Score', ascending=False), ax=ax1, palette='viridis')
        portfolio_avg_org_ai_r = filtered_df['Org_AI_R_Score'].mean()
        ax1.axhline(portfolio_avg_org_ai_r, color='red', linestyle='--',
                    label=f'Portfolio Average ({portfolio_avg_org_ai_r:.1f})')
        ax1.set_title(f'Org-AI-R Scores by Company ({selected_quarter})')
        ax1.set_ylabel('Org-AI-R Score')
        ax1.tick_params(axis='x', rotation=45)
        ax1.legend(title='Industry', bbox_to_anchor=(
        ...
        st.subheader(
            f"Org-AI-R Score vs. Industry-Adjusted Z-Score for {selected_quarter}")
        ...
        ax2.axvline(portfolio_mean_org_ai_r, color='gray', linestyle=':',
                    label=f'Portfolio Mean Org-AI-R ({portfolio_mean_org_ai_r:.1f})')

        # Industry mean Z-score (horizontal line at Y=0)
        ax2.axhline(0, color='black', linestyle='--',
                    label='Industry Mean Z-Score (0)')

        ax2.set_title(
            f'Org-AI-R Score vs. Industry-Adjusted Z-Score ({selected_quarter})')
        ...
        # Render an empty dataframe with expected columns for AppTest compatibility
        empty_df = pd.DataFrame(columns=['CompanyName', 'Industry', 'Org_AI_R_Score',
                                'Org_AI_R_Percentile', 'Org_AI_R_Z_Score']).set_index('CompanyName')
        st.dataframe(empty_df)
        st.info(
            "No data available for benchmarking. Please generate portfolio data first.")
...
# Page 4: AI Investment & EBITDA Impact
    ...
    **Attributed EBITDA Impact Percentage ($\Delta\text{EBITDA}\%$):**
    ...
    if not st.session_state.portfolio_df.empty:
        ...
        st.subheader(
            "AI Investment vs. Efficiency (Latest Quarter, Highlighting Attributed EBITDA Impact)")
        ...
        # Log scale for AI Investment for better distribution visibility
        ax3.set_xscale('log')
        ...
        st.dataframe(empty_df)
        st.info(
            "No data available for AI investment and EBITDA impact. Please generate portfolio data first.")
...
# Page 5: Tracking Progress Over Time
    ...
    if not st.session_state.portfolio_df.empty:
        all_companies = st.session_state.portfolio_df['CompanyName'].unique(
        ).tolist()
        ...
        if selected_companies:
            tracking_df = st.session_state.portfolio_df[st.session_state.portfolio_df['CompanyName'].isin(
                selected_companies)].copy()
            ...
            sns.lineplot(x='Quarter', y='Org_AI_R_Score', hue='CompanyName',
                         marker='o', data=tracking_df, ax=ax4, palette='deep')

            # Overlay portfolio average (added observed=False)
            portfolio_avg_df = st.session_state.portfolio_df.groupby(
                'Quarter', observed=False)['Org_AI_R_Score'].mean().reset_index()
            sns.lineplot(x='Quarter', y='Org_AI_R_Score', data=portfolio_avg_df, ax=ax4,
                         color='black', linestyle='--', label='Portfolio Average', marker='x')

            ax4.set_title('Org-AI-R Score Trajectory Over Time')
            ax4.set_xlabel('Quarter')
            ax4.set_ylabel('Org-AI-R Score')
            ax4.legend(title='Company / Average',
            ...
            # Plot individual company lines
            sns.lineplot(x='Quarter', y='AI_Investment_Efficiency', hue='CompanyName',
                         marker='o', data=tracking_df, ax=ax5, palette='dark')

            # Overlay portfolio average (added observed=False)
            portfolio_avg_aie_df = st.session_state.portfolio_df.groupby(
                'Quarter', observed=False)['AI_Investment_Efficiency'].mean().reset_index()
            sns.lineplot(x='Quarter', y='AI_Investment_Efficiency', data=portfolio_avg_aie_df,
                         ax=ax5, color='black', linestyle='--', label='Portfolio Average', marker='x')

            ax5.set_title('AI Investment Efficiency Trajectory Over Time')
            ax5.set_xlabel('Quarter')
            ax5.set_ylabel('AI Investment Efficiency')
            ax5.legend(title='Company / Average',
                       bbox_to_anchor=(1.05, 1), loc='upper left')
        ...
        st.info(
            "No data available for tracking progress. Please generate portfolio data first.")
...
# Page 6: Actionable Insights: CoE & Review
    ...
    coe_org_ai_r_threshold = st.slider("Org-AI-R Score Threshold for Center of Excellence", min_value=50, max_value=90, value=75, step=1,
                                       key="coe_org_ai_r_threshold", help="Companies with Org-AI-R score above this will be considered for 'Centers of Excellence'. Default: 75.")
    coe_ebitda_threshold = st.slider("EBITDA Impact (%) Threshold for Center of Excellence", min_value=1.0, max_value=10.0, value=3.0, step=0.5,
                                     key="coe_ebitda_threshold", help="Companies with EBITDA Impact above this will be considered for 'Centers of Excellence'. Default: 3%.")
    review_org_ai_r_threshold = st.slider("Org-AI-R Score Threshold for Companies for Review", min_value=20, max_value=70, value=50, step=1,
                                          key="review_org_ai_r_threshold", help="Companies with Org-AI-R score below or equal to this will be considered for 'Companies for Review'. Default: 50.")
    review_ebitda_threshold = st.slider("EBITDA Impact (%) Threshold for Companies for Review", min_value=0.0, max_value=5.0, value=1.0, step=0.1,
                                        key="review_ebitda_threshold", help="Companies with EBITDA Impact below or equal to this will be considered for 'Companies for Review'. Default: 1%.")

    re_evaluate_insights = st.button("Re-evaluate Actionable Insights", key="re_evaluate_insights_button",
                                     help="Click to re-identify Centers of Excellence and Companies for Review based on the adjusted thresholds.")
    ...
    if not st.session_state.portfolio_df.empty:
        ...
        if re_evaluate_insights or "centers_of_excellence_df" not in st.session_state or "companies_for_review_df" not in st.session_state:
            ...
            if re_evaluate_insights:
                st.success(
                    "Actionable insights re-evaluated with new thresholds.")
        ...
        **Centers of Excellence:** Portfolio companies with high Org-AI-R scores and significant EBITDA impact, serving as benchmarks for best practices and potential for replication across the fund.
        ...
        if not st.session_state.centers_of_excellence_df.empty:
            ...
            # Render an empty dataframe with expected columns for AppTest compatibility
            empty_df = pd.DataFrame(columns=['CompanyName', 'Industry', 'Org_AI_R_Score',
                                    'EBITDA_Impact', 'AI_Investment_Efficiency']).set_index('CompanyName')
            st.dataframe(empty_df)
            st.info(
                "No companies currently meet the 'Centers of Excellence' criteria with the current thresholds.")
        ...
        **Companies for Review:** Portfolio companies with lower Org-AI-R scores or minimal EBITDA impact, indicating areas requiring strategic intervention, additional resources, or a re-evaluation of AI strategy.
        ...
        if not st.session_state.companies_for_review_df.empty:
            ...
            st.dataframe(empty_df)
            st.info(
                "No companies currently meet the 'Companies for Review' criteria with the current thresholds.")

        # Visualization: Portfolio AI Performance: Org-AI-R Score vs. EBITDA Impact (Latest Quarter)
        st.subheader(
            "Portfolio AI Performance: Org-AI-R Score vs. EBITDA Impact (Latest Quarter)")
        latest_data_for_plot = st.session_state.portfolio_df.loc[st.session_state.portfolio_df.groupby(
            'CompanyID', observed=False)['Quarter'].idxmax()]
        ...
        for index, row in st.session_state.centers_of_excellence_df.iterrows():
            ax6.scatter(row['Org_AI_R_Score'], row['EBITDA_Impact'], marker='*',
                        s=1000, color='green', alpha=0.8, edgecolors='black')
            ax6.text(row['Org_AI_R_Score'] + 1, row['EBITDA_Impact'], row['CompanyName'],
        ...
        for index, row in st.session_state.companies_for_review_df.iterrows():
            ax6.scatter(row['Org_AI_R_Score'], row['EBITDA_Impact'],
                        marker='X', s=700, color='red', alpha=0.8, edgecolors='black')
            ax6.text(row['Org_AI_R_Score'] + 1, row['EBITDA_Impact'], row['CompanyName'],
                     fontsize=9, ha='left', va='center', color='red', weight='bold')

        # Add threshold lines
        ax6.axvline(coe_org_ai_r_threshold, color='green', linestyle=':',
                    label=f'CoE Org-AI-R Threshold ({coe_org_ai_r_threshold})')
        ax6.axhline(coe_ebitda_threshold, color='green', linestyle=':',
                    label=f'CoE EBITDA Impact Threshold ({coe_ebitda_threshold}%)')
        ax6.axvline(review_org_ai_r_threshold, color='red', linestyle='--',
                    label=f'Review Org-AI-R Threshold ({review_org_ai_r_threshold})')
        ax6.axhline(review_ebitda_threshold, color='red', linestyle='--',
                    label=f'Review EBITDA Impact Threshold ({review_ebitda_threshold}%)')
        ...
        ax6.legend(unique_labels_dict.values(), unique_labels_dict.keys(
        ), title='Metrics & Categories', bbox_to_anchor=(1.05, 1), loc='upper left')
        ...
        st.info(
            "No data available for actionable insights. Please generate portfolio data first.")
...
# Page 7: Exit-Readiness & Valuation
    ...
    *   $\delta$: The AI Premium Coefficient (`AI_PremiumCoefficient`), which determines how much each point of Exit-AI-R score contributes to the valuation multiple.
    ...
    w1 = st.slider("Weight for Visible AI Capabilities ($w_1$)", min_value=0.20, max_value=0.50, value=0.35, step=0.01, key="w1_slider",
    ...
    w2 = st.slider("Weight for Documented AI Impact ($w_2$)", min_value=0.20, max_value=0.50, value=0.40, step=0.01,
                   key="w2_slider", help="Emphasize quantified AI impact with clear audit trails. Default $w_2 = 0.40$.")
    w3 = st.slider("Weight for Sustainable AI Capabilities ($w_3$)", min_value=0.10, max_value=0.40, value=0.25, step=0.01,
                   key="w3_slider", help="Focus on embedded, long-term AI capabilities versus one-time projects. Default $w_3 = 0.25$.")

    # Optional: Display a warning if weights don't sum to near 1
    total_weights = w1 + w2 + w3
    if not (0.99 <= total_weights <= 1.01):
        st.warning(
            f"Note: Sum of weights (w1+w2+w3) is {total_weights:.2f}. Consider adjusting them to sum closer to 1 for typical weighted averages.")
    ...
    if st.button("Recalculate Exit-Readiness & Valuation", key="recalculate_exit_button", help="Click to re-compute Exit-AI-R scores and projected multiples with the selected weights."):
        st.session_state.portfolio_df = calculate_exit_readiness_and_valuation(
            st.session_state.portfolio_df, w1, w2, w3)
        st.success(
            "Exit-Readiness scores and projected valuations updated successfully!")
    ...
    if not st.session_state.portfolio_df.empty:
        ...
        st.subheader(
            "Exit-AI-R Score vs. Projected Exit Multiple (Latest Quarter)")
        ...
        # Render an empty dataframe with expected columns for AppTest compatibility
        empty_df = pd.DataFrame(columns=['CompanyName', 'Industry', 'Exit_AI_R_Score', 'BaselineMultiple',
                                'AI_Premium_Multiple_Additive', 'Projected_Exit_Multiple']).set_index('CompanyName')
        st.dataframe(empty_df)
        st.info(
            "No data available for exit-readiness and valuation. Please generate portfolio data first.")
